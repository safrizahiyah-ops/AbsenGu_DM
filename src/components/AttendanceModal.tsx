...
export const AttendanceModal: React.FC = () => {
  ...
  // Initialize or reset when modal opens or editingRecord changes
  useEffect(() => {
    if (isFormModalOpen) {
      ...
      if (editingRecord) {
        ...
        if (storedTarget && PERIOD_IDS.includes(storedTarget as PeriodId)) {
          ...
          const currentState = getCurrentPeriodState(currentTime);
  ...
  }, [isFormModalOpen, editingRecord, settings.currentPicketTeacher, currentTime, classes]);
